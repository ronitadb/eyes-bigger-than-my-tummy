...
async function update(req, res) {
  ...
  try {
    ...
    for (const f of TEXT) {
      ...
      if (f === 'external_url')  await sql`UPDATE articles SET external_url  = ${v} WHERE id = ${id}`;
    }
    ...
    if (typeof b.status !== 'undefined') {
      ...
      if (b.status === 'published') {
        ...
        const { rows } = await sql`
          SELECT title_lead, title_topic, jsonb_array_length(blocks) AS n FROM articles WHERE id = ${id}
        `;
        if (!rows.length) return res.status(404).json({ ok: false, error: 'not_found' });
        // Either title line will do: an article may drop its topic line.
        if (!(rows[0].title_lead || rows[0].title_topic) || !rows[0].n) {
          return res.status(400).json({ ok: false, error: 'incomplete',
            message: 'צריך כותרת ולפחות בלוק אחד לפני פרסום.' });
        }
