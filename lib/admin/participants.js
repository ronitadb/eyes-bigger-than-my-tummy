...
async function handlePut(req, res) {
  const { id, status, participant_type: participantType, first_name: firstName } = req.body || {};

  if (!id || (!status && typeof participantType === 'undefined' && typeof firstName === 'undefined')) {
    return res.status(400).json({ ok: false, error: 'missing id, status, participant_type or first_name' });
  }
