...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const participants = isStandalone
    ...
    for (const p of participants) {
      try {
        ...
        await sql`
          ...
          VALUES (${p.id}, ${isStandalone ? null : meetingId}, ${templateType}, 'sent')
        `;
        sent++;
      } catch (err) {
        ...
        await sql`
          ...
          VALUES (${p.id}, ${isStandalone ? null : meetingId}, ${templateType}, 'failed', ${err.message || 'unknown'})
        `;
        failed++;
      }
    }
